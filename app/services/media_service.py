...
class MediaService:
    ...
    async def upload_file(self, file: UploadFile, user_id: int, description: Optional[str] = None, is_public: bool = False) -> Dict[str, Any]:
        ...
        try:
            ...
            # Получаем конфигурацию для типа файла
            config = self.file_type_config[file_type]
    ...
    async def get_user_files(self, user_id: int, file_type: Optional[str] = None, limit: int = 50, offset: int = 0) -> List[Dict[str, Any]]:
    ...
    async def update_file(self, file_id: int, user_id: int, description: Optional[str] = None, is_public: Optional[bool] = None) -> bool:
    ...
    async def get_public_files(self, limit: int = 50, offset: int = 0) -> Dict[str, Any]:
        """Получает публичные файлы с пагинацией."""
        try:
